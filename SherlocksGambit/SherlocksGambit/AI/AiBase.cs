...
namespace SherlocksGambit.AI
{
    public abstract class AiBase
    {
        ...
        {
            return board.HasLost(Color) || board.HasWon(Color);
        }
        ...
        protected static List<PathObject> GeneratePaths(Board currBoard, PlayerColor currPlayer, bool enemyFlag = false)
        {
            ...
            bool attack = false;
            List<BasePiece> basePieces = currBoard.PieceManager.PiecesDictionary[currPlayer];

            for (int i = 0; i < basePieces.Count; i++)
            {
                int pos = currBoard.PieceManager.PiecesDictionary[currPlayer][i].CurrentCell.BoardPosition;
                var piece = currBoard.PieceManager.PiecesDictionary[currPlayer][i];
                List<PathObject> p = piece.CheckPathing(pos, null);
                
                if (enemyFlag)
                {
                    foreach (PathObject pathObject in p)
                    {
                        if (pathObject.Enemies.Count != 0)
        ...
        {
            if (!MoveEncryption.Decrypt(Board, move))
                return "WON!";
            ...
            Tuple<double, PathObject> next = GenerateMove(Board, MaxDepth);
            ElapsedTime = DateTime.Now.Ticks - start.Ticks;
            
            WithHeuristic = next.Item1;
            
            if (next.Item2 is null || next.Item2.Path.Count == 0)
                return "LOST...";
                
                
            return MoveEncryption.Encrypt(Color, next.Item2);
        }
    }
}