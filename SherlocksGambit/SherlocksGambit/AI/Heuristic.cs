...
namespace SherlocksGambit.AI
{
    public static class Heuristic
    {
        ...
        {
            return board.PieceManager.PiecesDictionary[currPlayer].Count -
                   board.PieceManager.PiecesDictionary[ColorHelper.GetOpponentColor(currPlayer)].Count;
        }
        ...
        {
            int count = 0;
            foreach (var basePiece in board.PieceManager.PiecesDictionary[currPlayer])
            {
                if (basePiece is King)
                    count += 2;
                if (basePiece is Man)
                    count += 1;
            }
            
            foreach (var basePiece in board.PieceManager.PiecesDictionary[ColorHelper.GetOpponentColor(currPlayer)])
            {
                if (basePiece is King)
                    count -= 2;
                if (basePiece is Man)
        ...
        public static double PositionWeightedHeuristics(Board board, PlayerColor currPlayer)
        {
            ...
            {
                if (basePiece is King)
                    count = count + 2 * PositionsWeights[basePiece.CurrentCell.BoardPosition - 1];
                if (basePiece is Man)
                    count += PositionsWeights[basePiece.CurrentCell.BoardPosition - 1];
            }
