...
namespace SherlocksGambit.Game
{
    public partial class Board
    {
        ...
        {
            Board b = new Board();
            b.PieceManager.PiecesDictionary[PlayerColor.White].Clear();
            b.PieceManager.PiecesDictionary[PlayerColor.Black].Clear();

            for (int i = 0; i < 32; i++)
            {
                if (Positions[i].CurrentPiece != null)
                {
                    b.Positions[i].CurrentPiece = Positions[i].CurrentPiece.Copy();
                    b.PieceManager.AddPiece(b.Positions[i].CurrentPiece);
                    b.Positions[i].CurrentPiece.Place(b.Positions[i]);
                }
                else
                {
                    b.Positions[i].CurrentPiece = null;
                }
            }
        ...
        public bool HasWon(PlayerColor playerColor)
        {
            ...
            {
                return PieceManager.PiecesDictionary[PlayerColor.White].Count == 0;
            }
            
            return PieceManager.PiecesDictionary[PlayerColor.Black].Count == 0;
        }
         ...
         * <returns> A boolean </returns>
         * If you remember the loosing conditions, one could lose having no pieces left on the Board or if none of the
         * player's remaining pieces are able to move
         * <remarks> Whether the opponent could move or not will not be checked here. </remarks>
         */
        public bool HasLost(PlayerColor playerColor)
        {
            // check if the player's pieces can more or not.
            if (playerColor == PlayerColor.Black)
            {
                if (PieceManager.PiecesDictionary[PlayerColor.Black].Count != 0)
                {
                    for (int i = 0; i < PieceManager.PiecesDictionary[PlayerColor.Black].Count; i++)
                    {
                        if (PieceManager.PiecesDictionary[PlayerColor.Black][i].CanMove(false))
                            return false;
                    }
                }
            ...
            if (PieceManager.PiecesDictionary[PlayerColor.White].Count != 0)
            {
                for (int i = 0; i < PieceManager.PiecesDictionary[PlayerColor.White].Count; i++)
                {
                    if (PieceManager.PiecesDictionary[PlayerColor.White][i].CanMove(false))
                        return false;
                }
            }
